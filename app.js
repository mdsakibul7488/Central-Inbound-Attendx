  ...
  document.getElementById('dashboard-content').innerHTML = `
    <div class="dash-header">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;">
        ...
        <div class="avatar-wrap" onclick="navigateTo('screen-settings');renderSettings();">
    ...
    <div style="margin:0 16px 24px;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
        ...
        <button class="view-all-btn" onclick="renderFullHistory();navigateTo('screen-history');">${t('lbl-view-all')}</button>
      </div>
...
function renderFullHistory() {
  ...
  function render(idx) {
    ...
    const rows = dates.reverse().map(date => {
      ...
      const selDay = dt.toLocaleDateString('en-US', {weekday:'long'});
      const weeklyOff = getWeeklyOffForEID(user.eid);
      const present = rec && rec.checkIn;
      const isDayOff = (weeklyOff && weeklyOff.toLowerCase()===selDay.toLowerCase()) || (rec && rec.status==='dayoff');
      ...
      return `<div class="day-row">
        <div>
          ...
          ${present ? `<div style="font-size:11px;color:var(--text-sub);">${rec.checkIn}${rec.checkOut?' – '+rec.checkOut:''}</div>` : ''}
        </div>
        <span class="status-pill ${pillClass}">${pillLabel}</span>
      </div>`;
    }).join('');
    ...
    document.getElementById('history-content').innerHTML = `
      <div class="month-tabs">
        ${cycles.map((c,i) => `<button class="month-tab ${i===idx?'active':''}" onclick="renderHistoryByIdx(${i})">${c.label}</button>`).join('')}
      </div>
      <div class="card" style="padding:0 16px;">${rows || '<div style="padding:16px;color:var(--text-sub);">No records found.</div>'}</div>`;
  }
  window.renderHistoryByIdx = render;
  render(0);
}
    ...
    <div style="margin-bottom:14px;">
      ...
      <div class="card">
        <div class="field"><label>New PIN</label>
          <div style="position:relative;"><input type="password" id="set-pin" placeholder="••••" maxlength="4" inputmode="numeric" style="padding-right:44px;"/>
          <button onclick="togglePass('set-pin',this)" type="button" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:16px;">👁</button></div>
    ...
    <div style="margin-bottom:14px;">
      ...
      <div class="card">
        <div class="field"><label>Current Password</label>
          <div style="position:relative;"><input type="password" id="set-old-pass" placeholder="Current password" style="padding-right:44px;"/>
          <button onclick="togglePass('set-old-pass',this)" type="button" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:16px;">👁</button></div>
        </div>
        <div class="field"><label>New Password</label>
          <div style="position:relative;"><input type="password" id="set-new-pass" placeholder="Min 8 chars, letters + numbers" style="padding-right:44px;"/>
          <button onclick="togglePass('set-new-pass',this)" type="button" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:16px;">👁</button></div>
...
function renderAdminDashboard() {
  ...
  users.filter(u=>u.status!=='blocked').forEach(u=>{
    ...
    const isWeeklyOff=weeklyOff&&weeklyOff.toLowerCase()===selDay.toLowerCase();
    if(rec&&rec.checkIn) presentCount++;
    else if(isWeeklyOff||(rec&&rec.status==='dayoff')) dayoffCount++;
    else absentCount++;
  });
  const pendingAtt=Math.max(0, totalResources-presentCount-dayoffCount);
  ...
  const empRows=users.map(u=>{
    ...
    return `<div class="emp-row" style="${isBlocked?'opacity:0.5':''}">
      ...
      <div style="flex:1;margin-left:8px;">
        ...
        <div style="font-size:11px;color:var(--text-sub);">${isPresent?`${rec.checkIn}${rec.checkOut?' – '+rec.checkOut:''}`:isDayOff?`Day off${weeklyOff?' ('+weeklyOff+')':''}`:isExchange?'Exchange':isBlocked?'Blocked':'No check in'}${isPresent&&rec.distanceFromOffice&&rec.distanceFromOffice!=='N/A'?' · 📍'+rec.distanceFromOffice:''}</div>
      </div>
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:3px;">
        ...
        ${isBlocked
          ?`<span onclick="unblockUser('${u.eid}');renderAdminDashboard();" style="font-size:10px;color:#0F6E56;background:var(--green-light);padding:2px 7px;border-radius:20px;cursor:pointer;">Unblock</span>`
        ...
        <span onclick="openResetPassword('${u.eid}','${u.name}')" style="font-size:10px;color:#6B7280;background:#F3F4F6;padding:2px 7px;border-radius:20px;cursor:pointer;">Reset Pass</span>
    ...
    <div class="admin-header">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;">
        ...
        <div style="position:relative;">
          ...
          <div id="shield-drop" style="position:absolute;top:46px;right:0;background:var(--surface);border-radius:var(--radius-sm);border:1px solid var(--border);box-shadow:0 8px 24px rgba(0,0,0,0.12);padding:6px;min-width:200px;z-index:100;display:none;">
      ...
      <div class="stats-grid">
        <div class="stat-box"><div class="stat-label">Total resources</div><div class="stat-val">${totalResources}</div></div>
...
// ADMIN HELPERS
// =============================================
function openResetPassword(eid, name) {
  ...
  el.innerHTML=`
    <div style="background:var(--surface);border-radius:var(--radius) var(--radius) 0 0;padding:24px;width:100%;max-width:430px;margin:0 auto;">
      <div style="font-size:16px;font-weight:700;margin-bottom:4px;">🔑 Reset Password</div>
      <div style="font-size:13px;color:var(--text-sub);margin-bottom:16px;">${name} · ${eid}</div>
      <div class="field"><label>New Password</label>
        <div style="position:relative;"><input type="password" id="reset-pass" placeholder="Min 8 chars, letters + numbers" style="width:100%;padding:13px 44px 13px 16px;border:1.5px solid var(--border);border-radius:var(--radius-sm);font-size:14px;outline:none;"/>
        <button onclick="togglePass('reset-pass',this)" type="button" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:16px;">👁</button></div>
        <div class="field-error" id="err-reset-pass"></div>
      </div>
      <button onclick="doResetPassword('${eid}')" class="btn btn-red" style="margin-bottom:8px;">Reset Password</button>
      <button onclick="this.closest('div[style*=fixed]').remove()" class="btn btn-ghost">Cancel</button>
    </div>`;
  document.body.appendChild(el);
}

function doResetPassword(eid) {
  const newPass=document.getElementById('reset-pass').value;
  const errEl=document.getElementById('err-reset-pass');
  const check=validatePassword(newPass);
  ...
  adminResetPassword(eid, newPass);
...
function openAdminPassChange() {
  ...
  el.innerHTML=`
    <div style="background:var(--surface);border-radius:var(--radius) var(--radius) 0 0;padding:24px;width:100%;max-width:430px;margin:0 auto;">
      ...
      <div class="field"><label>Current Password</label>
        <div style="position:relative;"><input type="password" id="admin-old-pass" placeholder="Current password" style="width:100%;padding:13px 44px 13px 16px;border:1.5px solid var(--border);border-radius:var(--radius-sm);font-size:14px;outline:none;"/>
        <button onclick="togglePass('admin-old-pass',this)" type="button" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:16px;">👁</button></div>
      </div>
      <div class="field"><label>New Password</label>
        <div style="position:relative;"><input type="password" id="admin-new-pass" placeholder="Min 8 chars, letters + numbers" style="width:100%;padding:13px 44px 13px 16px;border:1.5px solid var(--border);border-radius:var(--radius-sm);font-size:14px;outline:none;"/>
        <button onclick="togglePass('admin-new-pass',this)" type="button" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:16px;">👁</button></div>
...
function saveAdminPass() {
  ...
  if(!verifyAdminPassword(oldPass)){errEl.textContent='Current password is incorrect';errEl.classList.add('show');return;}
  const check=validatePassword(newPass);
  if(check==='short'){errEl.textContent=t('err-pass-short');errEl.classList.add('show');return;}
  if(check==='format'){errEl.textContent=t('err-pass-format');errEl.classList.add('show');return;}
...
function openLocationSetup() {
  ...
  el.innerHTML=`
    <div style="background:var(--surface);border-radius:var(--radius) var(--radius) 0 0;padding:24px;width:100%;max-width:430px;margin:0 auto;">
      ...
      <div class="field"><label>Check-in Radius (meters)</label>
...
function toggleShield() { const d=document.getElementById('shield-drop'); if(d)d.style.display=d.style.display==='none'?'block':'none'; }
function closeShield() { const d=document.getElementById('shield-drop'); if(d)d.style.display='none'; }
function openDatePicker() { const p=document.getElementById('date-picker'); if(p){p.value=adminSelectedDate;p.showPicker?p.showPicker():p.click();} }
...
function viewEmpHistory(eid, name) {
  ...
  function render(idx) {
    ...
    const rows=dates.reverse().map(date=>{
      ...
      const dayName=dt.toLocaleDateString('en-US',{weekday:'short',month:'short',day:'numeric'});
      const selDay=dt.toLocaleDateString('en-US',{weekday:'long'});
      const weeklyOff=getWeeklyOffForEID(eid);
      const present=rec&&rec.checkIn;
      const isDayOff=(weeklyOff&&weeklyOff.toLowerCase()===selDay.toLowerCase())||(rec&&rec.status==='dayoff');
      const isExchange=rec&&rec.status==='exchange';
      let pillClass='pill-absent',pillLabel='Absent';
    ...
    document.getElementById('admin-history-content').innerHTML=`
      <div class="month-tabs">${cycles.map((c,i)=>`<button class="month-tab ${i===idx?'active':''}" onclick="viewEmpHistoryByIdx(${i},'${eid}','${name}')">${c.label}</button>`).join('')}</div>
      <div class="card" style="padding:0 16px;">${rows||'<div style="padding:16px;color:var(--text-sub);">No records.</div>'}</div>`;
  }
  window.viewEmpHistoryByIdx=(idx,e,n)=>{document.getElementById('admin-history-sub').textContent=n+' · '+e;render(idx);};
  render(0);
  navigateTo('screen-admin-history');
}
